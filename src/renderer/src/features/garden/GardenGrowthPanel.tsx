import type { GardenGrowthSnapshot } from '../../types/emotion'

interface GardenGrowthPanelProps {
  snapshot: GardenGrowthSnapshot | null
  loading: boolean
}

function GardenGrowthPanel({ snapshot, loading }: GardenGrowthPanelProps): React.JSX.Element {
  if (!snapshot) {
    return (
      <section className="flex h-full items-center justify-center rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-surface)] p-5 text-sm text-[var(--text-muted)]">
        {loading ? '正在校准花园的成长状态…' : '还没有足够的花朵来评估成长状态。'}
      </section>
    )
  }

  return (
    <section
      className="flex h-full flex-col rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-surface)] p-5"
      data-garden-level={snapshot.level}
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-[var(--text-muted)]">花园成长</p>
          <h3 className="mt-2 text-2xl font-semibold text-[var(--text-primary)]">
            {snapshot.levelLabel}
          </h3>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="rounded-[2px] border-2 border-[var(--border-primary)] bg-[var(--bg-panel)] px-3 py-1 text-xs tracking-[0.2em] text-[var(--accent-rose)]">
            {snapshot.seasonalTheme?.combinedLabel ?? snapshot.seasonLabel}
          </span>
          <span className="text-[10px] text-[var(--text-muted)]">
            剩余浇水 {snapshot.manualWateringsRemaining ?? 0} 次/天
          </span>
        </div>
      </div>

      {snapshot.witheredCount > 0 && (
        <div className="mt-4 rounded-[4px] border-2 border-amber-500 bg-amber-100 px-3 py-2 text-xs text-amber-800">
          有 {snapshot.witheredCount} 朵花已枯萎，浇水可以让它们重新发芽。
        </div>
      )}

      <div className="mt-6 grid gap-3 md:grid-cols-2">
        <article className="rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-panel)] p-4">
          <p className="text-xs uppercase tracking-[0.22em] text-[var(--text-muted)]">当前连续</p>
          <p className="mt-3 text-3xl font-semibold text-[var(--text-primary)]">
            {snapshot.currentStreakDays} 天
          </p>
        </article>
        <article className="rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-panel)] p-4">
          <p className="text-xs uppercase tracking-[0.22em] text-[var(--text-muted)]">
            最近 7 天活跃
          </p>
          <p className="mt-3 text-3xl font-semibold text-[var(--text-primary)]">
            {snapshot.recentReleaseCount}
          </p>
        </article>
      </div>

      <div className="mt-5 rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-panel)] p-4">
        <div className="mb-3 flex items-center justify-between gap-3 text-sm text-[var(--text-secondary)]">
          <span>成长进度</span>
          <span>
            {snapshot.nextLevelLabel ? `下一阶段：${snapshot.nextLevelLabel}` : '已经进入最高阶段'}
          </span>
        </div>
        <div className="pixel-progress">
          <div
            className="pixel-progress-fill bg-gradient-to-r from-emerald-500 via-rose-400 to-amber-400"
            style={{ width: `${Math.round(snapshot.progressToNextLevel * 100)}%` }}
          />
        </div>
      </div>

      <div className="mt-5 rounded-[4px] border-2 border-[var(--border-primary)] bg-[var(--bg-panel)] p-4 text-sm leading-7 text-[var(--text-secondary)]">
        你的花园目前已经累计 {snapshot.totalBlooms} 朵花，最长连续释放 {snapshot.longestStreakDays}{' '}
        天。
        {snapshot.level === 1 && ' 继续保持几天，花园就会从新芽状态进入开花期。'}
        {snapshot.level === 2 && ' 你已经让花园稳定开花，再保持一段连续节律就能进入盛放期。'}
        {snapshot.level === 3 && ' 你的花园已经进入盛放状态，说明最近的情绪清理节律非常稳定。'}
      </div>
    </section>
  )
}

export default GardenGrowthPanel
